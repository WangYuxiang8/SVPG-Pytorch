...
class SVPG:
    ...
    def __init__(self, nagents, nparams, max_step_length, svpg_rollout_length, svpg_horizon, temperature, discrete,
                 kld_coefficient):
        self.particles = []
        ...
        self.nagents = nagents  # Number of agents(particles), each agent has a particle
        self.gamma = 0.99

        self.nparams = nparams  # Random params dimension
        self.noutputs = nparams * 2 if discrete else nparams
        self.discrete = discrete  # If the params is discrete, true - discrete; false - continuous
    ...
    def select_action(self, policy_idx, state):
        """
        Get the particle which index is 'policy_idx'. Get the action depending on current state.
        Args:
            policy_idx:
            state:
        Returns:
        ...
        policy = self.particles[policy_idx]     # select one particle to get the action distribution
        prior_policy = self.prior_particles[policy_idx]
        dist, value = policy(state)     # dist is action distribution, value is critic net output
        prior_dist, _ = prior_policy(state)

        action = dist.sample()      # sample from action distribution to get specific action

        policy.saved_log_probs.append(dist.log_prob(action))
        # Calculate prior action probability distribution and current's KL divergence
    ...
    def train(self, simulator_rewards):
        ...
        for i in range(self.nagents):
            ...
            # Calculate entropy-augmentled returns, advantages
    ...
    def load_prior_particles(self, directory):
        for i in range(self.nagents):
            self.prior_particles[i].load_state_dict(
                torch.load('{}/particle_{}.pth'.format(directory, i), map_location=device))
